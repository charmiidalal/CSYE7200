...
object Movie extends App {
  ...
  def elements(list: Seq[String], indices: Int*): List[String] = {
    ...
    // 6 points
    val result: Seq[String] =  for (index <- indices) yield Try(list(index)).getOrElse("")
    result.toList
  }
...
object Rating {
  ...
  def apply(s: String): Rating =  (for (ws <- rRating.unapplySeq(s)) yield for (w <- ws) yield Option(w))
    match {
    case Some(Seq(Some(contentRating), _, maybeAge)) => Rating(contentRating, maybeAge.map(_.toInt))
